...
namespace Vts.Test.MonteCarlo.Sources
{
    ...
    public class FluorescenceEmissionSourcesTests
    {
        ...
        public void verify_that_GetFinalPositionAndWeight_samples_from_CDFOfXAndYAndZ_correctly()
        {
            ...
            _tissue.Regions[3] = _xyzLoaderCDF.FluorescentTissueRegion;
            for (int i = 0; i < 100; i++)
            {
                var photon = _fluorEmissionAOfXAndYAndZSourceCDF.GetNextPhoton(_tissue);
        ...
        public void verify_that_GetFinalPositionAndWeight_samples_from_UnifOfXAndYAndZ_sampling_correctly()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                var photon = _fluorEmissionAOfXAndYAndZSourceUnif.GetNextPhoton(_tissue);
                // verify that photons start within range of midpoints of voxels in infinite cylinder
                Assert.IsTrue((photon.DP.Position.X >= -0.5) && (photon.DP.Position.X <= 0.5));
                Assert.AreEqual(photon.DP.Position.Y, 0);
                Assert.IsTrue((photon.DP.Position.Z >= 0.5) && (photon.DP.Position.Z <= 1.5));
        ...
        public void verify_that_GetFinalPositionAndWeight_samples_from_CDFOfRhoAndZ_correctly()
        {
            ...
            ITissue _tissue = _tissueInput.CreateTissue(AbsorptionWeightingType.Discrete,
                regionPhaseFunctions, 0);
            _tissue.Regions[3] = _rhozLoaderCDF.FluorescentTissueRegion;
            for (int i = 0; i < 100; i++)
            {
                var photon = _fluorEmissionAOfRhoAndZSourceCDF.GetNextPhoton(_tissue);
        ...
        {
            ITissueInput ti = new SingleInfiniteCylinderTissueInput(
            ...
            var _rng = RandomNumberGeneratorFactory.GetRandomNumberGenerator(
                RandomNumberGeneratorType.MersenneTwister, 0);
            for (int i = 0; i < ti.Regions.Length; i++)
            {
                if (!phaseFunctions.ContainsKey(ti.Regions[i].PhaseFunctionKey))
                {
                    phaseFunctions.Add(ti.Regions[i].PhaseFunctionKey,
                        PhaseFunctionFactory.GetPhaseFunction(ti.Regions[i], ti, _rng));
                }
            }
            ITissue tissue = ti.CreateTissue(AbsorptionWeightingType.Discrete,
                phaseFunctions, 0);
