...
namespace Vts.MonteCarlo.Sources.SourceProfiles
{
    ...
    public class ArbitrarySourceProfile : ISourceProfile
    {
        ...
        /// <param name="imageCenter">Position of image center (note z value not used)</param>
          public ArbitrarySourceProfile(double[] image, int numberOfPixelsWidthX, int numberOfPixelsHeightY, 
            double pixelSizeX, double pixelSizeY, Position imageCenter)
        {
            Image = image;
            ProbabilityDensityFunction = Image.Select(s => s / Image.Sum()).ToArray();
        ...
        {
            ProbabilityDensityFunction = new double[Image.Length];
            CumulativeDensityFunction = new double[Image.Length];
            var totalProb = Image.Sum();
            var runningCumulativeSum = 0.0;
            for (var i = 0; i < Image.Length - 1; i++)
            {
                ProbabilityDensityFunction[i] = Image[i] / totalProb;
                runningCumulativeSum += ProbabilityDensityFunction[i];
                CumulativeDensityFunction[i] = runningCumulativeSum;
            }
            CumulativeDensityFunction[Image.Length - 1] = 1.0; // set last point
        }
        /// <summary>
        ...
        { 
            ...
            for (var i = 0; i < Image.Length; i++)
            {
                if (rho >= CumulativeDensityFunction[i]) continue;
        ...
        /// </summary>
        public double[] ProbabilityDensityFunction { get; private set; }
        /// <summary>
        /// Cumulative density function
        /// </summary>
        public double[] CumulativeDensityFunction { get; private set; }
    }
}
