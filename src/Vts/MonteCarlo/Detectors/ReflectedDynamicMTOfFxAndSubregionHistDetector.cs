    ...
    public class ReflectedDynamicMTOfFxAndSubregionHistDetector : Detector, IDetector
    {
        ...
        public void Normalize(long numPhotons)
        {
            for (int ir = 0; ir < Fx.Count - 1; ir++)
            {
                ...
                {
                    Mean[ir, imt] /= numPhotons;
                    if (TallySecondMoment)
                    {
                        SecondMoment[ir, imt] /= numPhotons;
                    }
                    for (int ifrac = 0; ifrac < FractionalMTBins.Count + 1; ifrac++)
                    {
                        FractionalMT[ir, imt, ifrac] /= numPhotons;
                    } 
                }
                for (int iz = 0; iz < Z.Count - 1; iz++)
                {
                    TotalMTOfZ[ir, iz] /= numPhotons;
                    DynamicMTOfZ[ir, iz] /= numPhotons;
                    if (TallySecondMoment)
                    {
                        TotalMTOfZSecondMoment[ir, iz] /= numPhotons; 
                        DynamicMTOfZSecondMoment[ir, iz] /= numPhotons;              
                    }
                }
            }
