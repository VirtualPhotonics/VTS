...
namespace Vts.Modeling.ForwardSolvers.MGRTE._2D
{
    ...
    {
        public static double Pi = GlobalConstants.Pi;
        ...
        public static void MatrixSolver(double[,] A, double[] B, double[] sol)
        ...
        public static int FindTri(int tri, int[] pt1, int[] pt2)
        ...
        public static int FindTri2(int[] pt1, int[] pt2)
        ...
        public static double Residual(int nt, int ns, double[][][] d, double[] a)
                ...
                for (i = 0; i < ns; i++)
                {
                    ...
                    for (j = 0; j < nt; j++)
                    {
                        ...
                        {   // boundary outgoing flux (s dot n>0): add boundary contributions to left
                         ...
                         MultiGridCycle.MatrixSolver(left, right, flux[i][tri]);// update the nodal values at "tri"
                    }
                }
            }
                ...
                for (i = 0; i < ns; i++)
                {
                    ...
                    for (j = 0; j < nt; j++)
                    {
                        ...
                        for (k = 0; k < 3; k++)
                        {
                         ...
                         MultiGridCycle.MatrixSolver(left, right, flux[i][tri]);
                    }
                }
            }
        }

        private static void MatrixConvec(double a, double b, double[,] matrix)
        {
        ...
        private static void MatrixAbsorb(double a, double b, double c, double dettri, double[,] matrix)
        {
        ...
        private static void SourceAssign(double[] us, double[] temp, double[] right, double[] RHS, double dettri, double source_corr)
        {
            double a, b, c, d, e, f;
        ...
        public static void FtoC_a(int nt, int ns_c, double[][][] flux, double[][][] cflux)
        ...
        public static void CtoF_a(int nt, int ns_c, double[][][] flux, double[][][] dcflux)
        ...
        public static void FtoC_s(int nt_c, int ns, double[][][] flux, double[][][] cflux, int[][] smap, double[][][][] fc)
        ...
        public static void CtoF_s(int nt_c, int ns, double[][][] flux, double[][][] dcflux, int[][] smap, double[][][][] cf)
        ...
        public static void FtoC_s2(int nt_c, double[][] f, double[][] cf, int[][] smap, double[][][][] fc)
        // Purpose: this function describes spatial fine-to-coarse operator by L2 projection for piecewise linear DG.
        {
            int j, k, m, n;
            for (j = 0; j < nt_c; j++)
        ...
        private static void CtoF_s2(int nt_c, double[][] f, double[][] cf2, int[][] smap, double[][][][] cf)
        // Purpose: this function describes spatial coarse-to-fine operator by linear interpolation for piecewise linear DG.
        {
            int j, k, m, n;
            for (j = 0; j < nt_c; j++)
        ...
        public static void FtoC(int nt_c, int ns_c, double[][][] flux, double[][][] cflux, int[][] smap, double[][][][] fc)
        ...
        public static void CtoF(int nt_c, int ns_c, double[][][] flux, double[][][] dcflux, int[][] smap, double[][][][] cf)
        ...
        public static void SpatialMapping(SpatialMesh cmesh, SpatialMesh fmesh, int[][] smap)
        ...
        //          in particular, it is the same with respect to different intervals.
        {
            ...
            for (i = 0; i < nt_f; i++)
            {
                ...
                for (j = 0; j < nt_c; j++)
                {
                    tri =  MultiGridCycle.FindMin(nt_c, distance);// find the triangle with minimal distance between (x,y) and centers of coarse triangles.
                    distance[tri] = 1e10;
                    x1 = p[t[tri][0]][0]; y1 = p[t[tri][0]][1];
                    x2 = p[t[tri][1]][0]; y2 = p[t[tri][1]][1];
                    x3 = p[t[tri][2]][0]; y3 = p[t[tri][2]][1];

                    area1 =  MultiGridCycle.Area(x, y, x2, y2, x3, y3);
                    area2 =  MultiGridCycle.Area(x1, y1, x, y, x3, y3);
                    area3 =  MultiGridCycle.Area(x1, y1, x2, y2, x, y);
        ...
        public static void SpatialMapping2(SpatialMesh cmesh, SpatialMesh fmesh, int[][] smap, double[][][][] cf, double[][][][] fc, int tempsize)
        ...
        //          the 4th indexes 3 fine nodes in the fine triangle.
        {
            ...
            for (i = 0; i < nt_c; i++)
            {
                ...
                tempd =  MultiGridCycle.Area(x1, y1, x2, y2, x3, y3);
                for (j = 0; j < np; j++)
                {
                    x = pf[temp[j][0]][0]; y = pf[temp[j][0]][1];
                    temp2[j][1] =  MultiGridCycle.Area(x1, y1, x, y, x3, y3) / tempd;
                    temp2[j][2] =  MultiGridCycle.Area(x1, y1, x2, y2, x, y) / tempd;
        ...
        public static void BoundReflection(int ns, double[][] theta, SpatialMesh smesh, double index_i, double index_o, BoundaryCoupling b)
        {
        ...
        public static void Bound(int ne, int nt, int[][] t, int[][] p2, double[][] p, int[][] e, int[][] e2, int[][] so2, double[][] n, int[] ori)
        ...
        //          ori[ne]:    orientation of the boundary edge w.r.t. the triangle, which is useful at ONLY one place in "bc_reflection".
        {
            ...
            for (i = 0; i < ne; i++)
            {
                tri =  MultiGridCycle.FindTri2(p2[e[i][1]], p2[e[i][2]]);
                e[i][0] = tri;
        ...
        public static void EdgeTri(int nt, double[] theta, double[][] p, int[][] p2, int[][] t, int[][] bd, double[][] bd2, int[][] so2)
        ...
        // Note:    we define "23" as the 1st edge, "31" as the 2nd and "12" as the 3rd.
        {
            ...
            for (i = 0; i < nt; i++)
            {
                ...
                {
                    bd[i][0] =  MultiGridCycle.FindTri(i, p2[t[i][1]], p2[t[i][2]]);// to find the adjacent triangle
                    if (bd[i][0] > -1)
                    {
                        for (j = 0; j < 3; j++)
                        {
                ...
                {
                    bd[i][3] =  MultiGridCycle.FindTri(i, p2[t[i][0]], p2[t[i][2]]);
                    if (bd[i][3] > -1)
                    {
                        for (j = 0; j < 3; j++)
                        {
                ...
                {
                    bd[i][6] =  MultiGridCycle.FindTri(i, p2[t[i][0]], p2[t[i][1]]);
                    if (bd[i][6] > -1)
                    {
                        for (j = 0; j < 3; j++)
                        {
