...
namespace Vts.FemModeling.MGRTE._2D.IO
{
    class Source
    {
        ...
        //              2.2. assembly boundary source into "q"
        {
            ...
            //Todo: Hard coded source file for July 26meeting. Correct it later
            ...
            if (Source.S_TYPE != NO_SOURCE)
            {
                ...
                else
                {
                    ...
                    for (i = 0; i < Source.n; i++)
                    {
                        ...
                        for (j = 0; j < nt; j++)
                        {
                            x1 = smesh[sMeshLevel].p[smesh[sMeshLevel].t[j][0]][0];
                            x2 = smesh[sMeshLevel].p[smesh[sMeshLevel].t[j][1]][0];
                            x3 = smesh[sMeshLevel].p[smesh[sMeshLevel].t[j][2]][0];
                            y1 = smesh[sMeshLevel].p[smesh[sMeshLevel].t[j][0]][1];
                            y2 = smesh[sMeshLevel].p[smesh[sMeshLevel].t[j][1]][1];
                            y3 = smesh[sMeshLevel].p[smesh[sMeshLevel].t[j][2]][1];
                                            ...
                                            if (l3 >= 0.0)
                                            {
                                                if (l3 <= 1.0)
                                                {
                                                    ...
                                                    for (k = 0; k < 3; k++)
                                                    {
                                                        for (l = 0; l < ns; l++)
                                                        {
                                                            RHS[level][l][j][k] = area[k] * s_int[i];
                                                        }
                                                    }
                                                }
                                            }
