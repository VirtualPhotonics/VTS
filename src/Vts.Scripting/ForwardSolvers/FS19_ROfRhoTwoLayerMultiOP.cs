...
public class FS19_ROfRhoTwoLayerMultiOP : IDemoScript
{
    ...
    public static void RunDemo()
    {
        ...
        Chart.Combine(
            new[] {
                LineChart(rhos, rOfRho1.Select(r => Math.Log(r)).ToArray(), xLabel: "fx [mm-1]", yLabel: $"R(rho) [mm-2]",
                    title: $"log(R(rho) [mm-2])@ mua1={op1[0].RegionOP.Mua:F3}"),
                LineChart(rhos, rOfRho2.Select(r => Math.Log(r)).ToArray(), xLabel: "fx [mm-1]", yLabel: $"R(rho) [mm-2]",
                    title: $"log(R(rho) [mm-2])@ mua1={op2[0].RegionOP.Mua:F3}"),
                LineChart(rhos, rOfRho3.Select(r => Math.Log(r)).ToArray(), xLabel: "fx [mm-1]", yLabel: $"R(rho) [mm-2]",
