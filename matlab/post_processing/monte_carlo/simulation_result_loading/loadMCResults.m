...
for di = 1:numDetectors
    if isfield(xml.DetectorInputs(di),'anyType')
        detector = xml.DetectorInputs(di).anyType;
    else        
        detector = xml.DetectorInputs(di).IDetectorInput;
    end
    switch(detector.TallyType)
        case 'RDiffuse'
            RDiffuse.Name = detector.Name;
            RDiffuse_xml = xml_load([datadir slash detector.Name '.xml']);
        ...
        case 'TDiffuse'
            ...
            TDiffuse_xml = xml_load([datadir slash detector.Name '.xml']);
        ...
        case 'ATotal'
            ...
            ATotal_xml = xml_load([datadir slash detector.Name '.xml']);
